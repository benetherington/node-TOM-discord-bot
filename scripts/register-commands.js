...
const provisionSlashes = async (slashes) => {
    try {
        ...
        const registeredSlashes = await rest.put(Routes.applicationGuildCommands(ID.user.bot, ID.guild.tomCast), {
            body: slashes,
        });
        ...
        await rest.put(Routes.guildApplicationCommandsPermissions(ID.user.bot, ID.guild.tomCast), {
